...
def home_recipes(request):
    ...
    return render(request, 'recipes/home_page.html', {'recipes': recipes, 'recipe': recipe})
...
def recipe_form(request):
    ...
    if user.is_authenticated:
        form = RecipeForm(request.POST or None, request.FILES or None, initial={'recipe_author': get_user_model().username})
        ...
        # return redirect(recipe_details(request,id))
        # context = Recipe.objects.filter(author=User,username)
        # context = {'recipe': Recipe.objects.get(id=id)}
        # return render(request, 'recipes/recipe_detail.html', context)
        context['form']=form
...
def edit_recipe(request, id):
    ...
    if request.method == 'POST':
        edit_form = RecipeEditForm(request.POST, request.FILES, instance=recipe_to_edit)
        if edit_form.is_valid():
            edit_form.save()
            messages.success(request,'You have edit the recipe')
            return redirect('recipe by author', id=id)
...
def delete_recipe(request,id):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'recipes/confirm_recipe_deletion.html',
                      {'object': 'Recipe', 'recipe_to_delete': recipe_to_delete})
